...
#                           Default Parameters
##############################################################
udmInfo = UDMInfo( name = "RM_Core",
                    company = "ANSYS, Inc.",
                    purpose = "Create RM Core and Windings",
...
#---------------------------------------------------------------------------------       
    def Refresh(self, funcLib, udmInParams, updatedParams, refreshModifiedPartsOnly, nonEditedPartRefIds):
        ...
        Dia = DimD7/math.sqrt(2)
        
        vertices.append(UDPPosition(-DimD1/2,(Dia-(DimD1/2)),-AirGapS-TAirGap))
        vertices.append(UDPPosition(-(Dia/2),(Dia/2),-AirGapS-TAirGap))
        vertices.append(UDPPosition(-(DimD8/2),(DimD8/2),-AirGapS-TAirGap))
        ...
        XBaseCore = funcLib.CreatePolyline(polylineDefinition)
        
        SweepVect = UDPVector(0,0,-(DimD4/2)+AirGapS)
        sweepOptions = UDPSweepOptions(SweepDraftType.RoundDraft, 0.0, 0.0)
        
        XBase = funcLib.SweepAlongVector(XBaseCore,SweepVect,sweepOptions)
        ...
        if DimD6 != 0:
            Cyl3Pos = UDPPosition(0,0,-(DimD4/2)-TAirGap)
            Cyl3Sta = UDPPosition(-(DimD6/2), 0, -(DimD4/2)-TAirGap)
            XCyl3 = funcLib.CreatePolyhedron(CoordinateSystemAxis.ZAxis,Cyl3Pos, Cyl3Sta, NumSegs,DimD4/2)
            
            funcLib.Subtract([XBaseCore],[XCyl3])
        
        funcLib.SetPartName("RM_Core_Bottom",XBaseCore)
        funcLib.SetPartColor(XBaseCore,0xa52a2a)
        MyUDMUtils.SetAttributes(XBaseCore, "RM_Core_Bottom", funcLib)
        #MyUDMUtils.SetPartAttributes(XBaseCore,"RM_Core_Bottom", "ferrite", 0xa52a2a, funcLib, "RM_Core_Bottom")
        MirrorVect = UDPVector(0,0,1)
        MirrorPt = UDPPosition(0,0,0)
        TopCore = funcLib.DuplicateAndMirror(XBaseCore,MirrorPt,MirrorVect)
        
        funcLib.SetPartName("RM_Core_Top",TopCore)
        funcLib.SetPartColor(TopCore,0xa52a2a)
        MyUDMUtils.SetAttributes(TopCore, "RM_Core_Top", funcLib)
        #MyUDMUtils.SetPartAttributes(TopCore,"RM_Core_Top", "ferrite", 0xa52a2a, funcLib, "RM_Core_Top")
