...
#                           Default Parameters
##############################################################
udmInfo = UDMInfo( name = "UI_Core",
                    company = "ANSYS, Inc.",
                    purpose = "Create UI Core and Windings",
...
#---------------------------------------------------------------------------------       
    def Refresh(self, funcLib, udmInParams, updatedParams, refreshModifiedPartsOnly, nonEditedPartRefIds):
        ...
        if self.CheckCoreDim([DimD1,DimD2,DimD3,DimD4,DimD5]) == 0:
        ...
        BaseCorePos = UDPPosition(-(DimD1-DimD2)/4,-(MECoreWidth)/2,(DimD4/2)-DimD3-TAirGap)
        BaseCoreSize=[MECoreLength, MECoreWidth, MECoreHeight]

        XBaseCore = funcLib.CreateBox(BaseCorePos, BaseCoreSize)
        
        SlotPos = UDPPosition((DimD1-DimD2)/4, -(MECoreWidth)/2, -(DimD4/2)-TAirGap)
        SlotSz = [DimD2, MECoreWidth, DimD4]
        
        XSlot = funcLib.CreateBox(SlotPos, SlotSz)
        
        funcLib.Subtract([XBaseCore],[XSlot])
        
        if AirGapC > 0:
            AgBoxPos = UDPPosition(-(DimD1-DimD2)/4,-(MECoreWidth)/2,(DimD4/2)-AirGapC)
            AgBoxSz = [(DimD1-DimD2)/2,MECoreWidth,AirGapC]
            AgC = funcLib.CreateBox(AgBoxPos, AgBoxSz)
            funcLib.Subtract([XBaseCore],[AgC])
        if AirGapS > 0:
            AgBoxPos2 = UDPPosition(DimD2+(DimD1-DimD2)/4,-(MECoreWidth)/2,(DimD4/2)-AirGapS)
            AgBoxSz2 = [(DimD1-DimD2)/2,MECoreWidth,AirGapS]
            AgS = funcLib.CreateBox(AgBoxPos2, AgBoxSz2)
            funcLib.Subtract([XBaseCore],[AgS])
        
        funcLib.SetPartName("U_Core",XBaseCore)
        funcLib.SetPartColor(XBaseCore,0xa52a2a)
        MyUDMUtils.SetAttributes(XBaseCore, "U_Core", funcLib)
        #MyUDMUtils.SetPartAttributes(XBaseCore,"U_Core", "ferrite", 0xa52a2a, funcLib, "U_Core")
        
        ICorPos = UDPPosition(-((DimD1-DimD2)/4)+((DimD1-DimD6)/2),-DimD7/2,(DimD4/2)+TAirGap)
        ICorSz = [DimD6, DimD7, DimD8]
        
        XICor = funcLib.CreateBox(ICorPos, ICorSz)
        
        funcLib.SetPartName("I_Core",XICor)
        funcLib.SetPartColor(XICor,0xa52a2a)
        MyUDMUtils.SetAttributes(XICor, "I_Core", funcLib)
        #MyUDMUtils.SetPartAttributes(XICor,"I_Core", "ferrite", 0xa52a2a, funcLib, "I_Core")
        
        CreateWdg = self._GetParamValueByName('WdgStatus')
        if CreateWdg == 1:
            if self.DrawWdg(DimD1,DimD2,DimD3/2,(DimD4/2)+TAirGap,DimD5,SAng,funcLib) == 0:
...
#-----------------------------------------------------------------------
    ...
    def DrawWdg(self,DimD1,DimD2,DimD3,DimD4,DimD5,SAng,funcLib):
        ...
        if BobStat > 0:
            self.DrawBobbin(MSlotHeight-2*MTopMargin,((LegWidth/2)+DimD2)-MSideMargin,(LegWidth/2.0)+MSideMargin+MBobbinThk,(DimD5/2.0)+MSideMargin+MBobbinThk,MBobbinThk,MSideMargin,SAng,funcLib)
        ...
        if MWdgType == 1:
            ...
            for MAx in self.WdgParDict:
                for MBx in range(0,int(self.WdgParDict[MAx][2])):
                    ...
                    if MCondType == 1:
                        ...
                        self.CreateSingleTurn2(MRecSzX,MRecSzY,MRecSzZ,MCondType,self.WdgParDict[MAx][0],self.WdgParDict[MAx][1],DimD4-MTDx-(self.WdgParDict[MAx][3]),MAx,MBx,(MRecSzX-LegWidth)/2,SAng,funcLib)
                    else:
                        MRecSzZ = -self.WdgParDict[MAx][0]/2.0
                        self.CreateSingleTurn(MRecSzX,MRecSzY,MRecSzZ,MCondType,self.WdgParDict[MAx][0],self.WdgParDict[MAx][1],DimD4-MTDx-(self.WdgParDict[MAx][3]),MAx,MBx,(MRecSzX-LegWidth)/2,SAng,funcLib)
                if MCondType == 1:
            ...
            for MAx in self.WdgParDict.keys():
                for MBx in range(0,int(self.WdgParDict[MAx][2])):
                    ...
                    if MCondType == 1:
                        MRecSzZ = -self.WdgParDict[MAx][1]/2.0
                        self.CreateSingleTurn2(MRecSzX,MRecSzY,MRecSzZ,MCondType,self.WdgParDict[MAx][0],self.WdgParDict[MAx][1],DimD4-MTopMargin-MBobbinThk-(self.WdgParDict[MAx][3])-MBx*(2*self.WdgParDict[MAx][3]+self.WdgParDict[MAx][1]),MAx,MBx,(MRecSzX-LegWidth)/2,SAng,funcLib)
                    else:
                        MRecSzZ = -self.WdgParDict[MAx][0]/2.0
                        self.CreateSingleTurn(MRecSzX,MRecSzY,MRecSzZ,MCondType,self.WdgParDict[MAx][0],self.WdgParDict[MAx][1],DimD4-MTopMargin-MBobbinThk-(self.WdgParDict[MAx][3])-MBx*(2*self.WdgParDict[MAx][3]+self.WdgParDict[MAx][0]),MAx,MBx,(MRecSzX-LegWidth)/2,SAng,funcLib)
                MTDx = MTDx + MLSpacing + self.WdgParDict[MAx][0] + 2*self.WdgParDict[MAx][3]
            return 1
