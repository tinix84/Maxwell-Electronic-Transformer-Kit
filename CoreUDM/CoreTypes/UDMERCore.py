...
#                           Default Parameters
##############################################################
udmInfo = UDMInfo( name = "ER_Core",
                    company = "ANSYS, Inc.",
                    purpose = "Create ER Core and Windings",
...
#---------------------------------------------------------------------------------       
    def Refresh(self, funcLib, udmInParams, updatedParams, refreshModifiedPartsOnly, nonEditedPartRefIds):
        ...
        if DimD7 > 0:
            SLBoxPos = UDPPosition(-(DimD7)/2,-(MECoreWidth)/2,-MSlotDepth-TAirGap)
            SLBoxSz = [DimD7,MECoreWidth,MSlotDepth]
            SLBox = funcLib.CreateBox(SLBoxPos, SLBoxSz)
            funcLib.Subtract([XBaseCore],[SLBox])
        ...
        funcLib.SetPartName("ER_Core_Bottom",XBaseCore)
        funcLib.SetPartColor(XBaseCore,0xa52a2a)
        MyUDMUtils.SetAttributes(XBaseCore, "ER_Core_Bottom", funcLib)
        #MyUDMUtils.SetPartAttributes(XBaseCore,"ER_Core_Bottom", "ferrite", 0xa52a2a, funcLib, "ER_Core_Bottom")
        MirrorVect = UDPVector(0,0,1)
        MirrorPt = UDPPosition(0,0,0)
        TopCore = funcLib.DuplicateAndMirror(XBaseCore,MirrorPt,MirrorVect)
        
        funcLib.SetPartName("ER_Core_Top",TopCore)
        funcLib.SetPartColor(TopCore,0xa52a2a)
        MyUDMUtils.SetAttributes(TopCore, "ER_Core_Top", funcLib)
        #MyUDMUtils.SetPartAttributes(TopCore,"ER_Core_Top", "ferrite", 0xa52a2a, funcLib, "ER_Core_Top")
