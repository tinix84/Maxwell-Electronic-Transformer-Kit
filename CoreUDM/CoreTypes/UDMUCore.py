...
#                           Default Parameters
##############################################################
udmInfo = UDMInfo( name = "U_Core",
                    company = "ANSYS, Inc.",
                    purpose = "Create U Core and Windings",
...
#---------------------------------------------------------------------------------       
    def Refresh(self, funcLib, udmInParams, updatedParams, refreshModifiedPartsOnly, nonEditedPartRefIds):
        ...
        BaseCorePos = UDPPosition(-(DimD1-DimD2)/4,-(MECoreWidth)/2,-MECoreHeight-TAirGap)
        BaseCoreSize=[MECoreLength, MECoreWidth, MECoreHeight]

        XBaseCore = funcLib.CreateBox(BaseCorePos, BaseCoreSize)
        
        SlotPos = UDPPosition((DimD1-DimD2)/4, -(MECoreWidth)/2, -DimD4-TAirGap)
        ...
        if AirGapC > 0:
            AgBoxPos = UDPPosition(-(DimD1-DimD2)/4,-(MECoreWidth)/2,-AirGapC)
            AgBoxSz = [(DimD1-DimD2)/2,MECoreWidth,AirGapC]
            AgC = funcLib.CreateBox(AgBoxPos, AgBoxSz)
            funcLib.Subtract([XBaseCore],[AgC])
        if AirGapS > 0:
            AgBoxPos2 = UDPPosition(DimD2+(DimD1-DimD2)/4,-(MECoreWidth)/2,-AirGapS)
            AgBoxSz2 = [(DimD1-DimD2)/2,MECoreWidth,AirGapS]
            AgS = funcLib.CreateBox(AgBoxPos2, AgBoxSz2)
            funcLib.Subtract([XBaseCore],[AgS])
        
        funcLib.SetPartName("U_Core_Bottom",XBaseCore)
        funcLib.SetPartColor(XBaseCore,0xa52a2a)
        MyUDMUtils.SetAttributes(XBaseCore, "U_Core_Bottom", funcLib)
        #MyUDMUtils.SetPartAttributes(XBaseCore,"U_Core_Bottom", "ferrite", 0xa52a2a, funcLib, "U_Core_Bottom")
        MirrorVect = UDPVector(0,0,1)
        MirrorPt = UDPPosition(0,0,0)
        TopCore = funcLib.DuplicateAndMirror(XBaseCore,MirrorPt,MirrorVect)
        
        funcLib.SetPartName("U_Core_Top",TopCore)
        funcLib.SetPartColor(TopCore,0xa52a2a)
        MyUDMUtils.SetAttributes(TopCore, "U_Core_Top", funcLib)
        #MyUDMUtils.SetPartAttributes(TopCore,"U_Core_Top", "ferrite", 0xa52a2a, funcLib, "U_Core_Top")
        CreateWdg = self._GetParamValueByName('WdgStatus')
        if CreateWdg == 1:
            if self.DrawWdg(DimD1,DimD2,DimD3,DimD4+TAirGap,DimD5,SAng,funcLib) == 0:
...
#-----------------------------------------------------------------------
    ...
    def CreateSingleTurn2(self,PathX,PathY,PathZ,ProfTyp,ProfAX,ProfZ,ZPos,LayNum,TurnNum,FRad,funcLib):
