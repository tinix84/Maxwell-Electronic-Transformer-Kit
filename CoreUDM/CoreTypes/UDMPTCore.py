...
#                           Default Parameters
##############################################################
udmInfo = UDMInfo( name = "PT_Core",
                    company = "ANSYS, Inc.",
                    purpose = "Create PT Core and Windings",
...
#---------------------------------------------------------------------------------       
    def Refresh(self, funcLib, udmInParams, updatedParams, refreshModifiedPartsOnly, nonEditedPartRefIds):
        ...
        BaseCorePos = UDPPosition(0,0,-(DimD4/2)-TAirGap)
        BaseCoreSta= UDPPosition(-DimD1/2, 0, -(DimD4/2)-TAirGap)
        XBaseCore = funcLib.CreatePolyhedron(CoordinateSystemAxis.ZAxis,BaseCorePos, BaseCoreSta, NumSegs,(DimD4/2)-AirGapS)
        
        Cyl1Pos = UDPPosition(0,0,-(DimD5/2)-TAirGap)
        Cyl1Sta = UDPPosition(-(DimD2/2),0,-(DimD5/2)-TAirGap)
        XCyl1 = funcLib.CreatePolyhedron(CoordinateSystemAxis.ZAxis,Cyl1Pos, Cyl1Sta, NumSegs,DimD5/2)
        
        funcLib.Subtract([XBaseCore],[XCyl1])
        
        Cyl2Sta = UDPPosition(-(DimD3/2),0,-(DimD5/2)-TAirGap)
        XCyl2 = funcLib.CreatePolyhedron(CoordinateSystemAxis.ZAxis,Cyl1Pos, Cyl2Sta, NumSegs,(DimD5/2)-AirGapC)
            
        funcLib.Unite([XBaseCore,XCyl2])
        
        if DimD6 != 0 :
            Cyl3Sta = UDPPosition(-(DimD6/2),0,-(DimD4/2)-TAirGap)
            XCyl3 = funcLib.CreatePolyhedron(CoordinateSystemAxis.ZAxis,BaseCorePos, Cyl3Sta, NumSegs,DimD4/2)
        
            funcLib.Subtract([XBaseCore],[XCyl3])
        ...
        if DimD7 != 0:
            Slot1Pos = UDPPosition(-DimD1/2, -DimD7/2, -(DimD4/2)-TAirGap)
            Slot1Sz = [(DimD1-DimD8)/2, DimD7,DimD4/2]
            
            Slot1 = funcLib.CreateBox(Slot1Pos, Slot1Sz)
            
            Slot2Pos = UDPPosition(DimD1/2, -DimD7/2, -(DimD4/2)-TAirGap)
            Slot2Sz = [-(DimD1-DimD8)/2, DimD7,DimD4/2]
            
            Slot2 = funcLib.CreateBox(Slot2Pos, Slot2Sz)
            
            funcLib.Subtract([XBaseCore],[Slot1,Slot2]) 
        
        Slot3Pos = UDPPosition(-DimD1/2, -DimD1/2, (DimD4/2)+TAirGap)
        Slot3Sz = [(DimD1-DimD8)/2, DimD1,-DimD4/2]
        
        Slot3 = funcLib.CreateBox(Slot3Pos, Slot3Sz)
        
        Slot4Pos = UDPPosition(DimD1/2, -DimD1/2, (DimD4/2)+TAirGap)
        Slot4Sz = [-(DimD1-DimD8)/2, DimD1,-DimD4/2]
        
        Slot4 = funcLib.CreateBox(Slot4Pos, Slot4Sz)
        
        funcLib.Subtract([TopCore],[Slot3,Slot4]) 
        
        funcLib.SetPartName("PT_Core_Bottom",XBaseCore)
        funcLib.SetPartColor(XBaseCore,0xa52a2a)
        MyUDMUtils.SetAttributes(XBaseCore, "PT_Core_Bottom", funcLib)
        funcLib.SetPartName("PT_Core_Top",TopCore)
        funcLib.SetPartColor(TopCore,0xa52a2a)
        MyUDMUtils.SetAttributes(TopCore, "PT_Core_Top", funcLib)
        #MyUDMUtils.SetPartAttributes(XBaseCore,"PT_Core_Bottom", "ferrite", 0xa52a2a, funcLib, "PT_Core_Bottom")
        #MyUDMUtils.SetPartAttributes(TopCore,"PT_Core_Top", "ferrite", 0xa52a2a, funcLib, "PT_Core_Top")
...
#-----------------------------------------------------------------------
    ...
    def DrawWdg(self,DimD2,DimD3,DimD5,DimD6,SAng,funcLib):
        ...
        MSlotHeight = DimD5*2
        if BobStat > 0:
            self.DrawBobbin(MSlotHeight-2*MTopMargin,(DimD2/2.0)-MSideMargin,(DimD3/2.0)+MSideMargin+MBobbinThk,MBobbinThk,SAng,funcLib)
        ...
        if MWdgType == 1:
            ...
            for MAx in self.WdgParDict:
                for MBx in range(0,int(self.WdgParDict[MAx][2])):
                    ...
                    self.CreateSingleTurn(MRecSzX,MRecSzZ,MCondType,self.WdgParDict[MAx][0],self.WdgParDict[MAx][1],DimD5-MTDx-(self.WdgParDict[MAx][3]),MAx,MBx,SAng,funcLib)
                if MCondType == 1:
            ...
            for MAx in self.WdgParDict.keys():
                for MBx in range(0,int(self.WdgParDict[MAx][2])):
                    ...
                    if MCondType == 1:
                        MRecSzZ = -self.WdgParDict[MAx][1]/2.0
                        self.CreateSingleTurn(MRecSzX,MRecSzZ,MCondType,self.WdgParDict[MAx][0],self.WdgParDict[MAx][1],DimD5-MTopMargin-MBobbinThk-(self.WdgParDict[MAx][3])-MBx*(2*self.WdgParDict[MAx][3]+self.WdgParDict[MAx][1]),MAx,MBx,SAng,funcLib)
                    else:
                        MRecSzZ = -self.WdgParDict[MAx][0]/2.0
                        self.CreateSingleTurn(MRecSzX,MRecSzZ,MCondType,self.WdgParDict[MAx][0],self.WdgParDict[MAx][1],DimD5-MTopMargin-MBobbinThk-(self.WdgParDict[MAx][3])-MBx*(2*self.WdgParDict[MAx][3]+self.WdgParDict[MAx][0]),MAx,MBx,SAng,funcLib)
                MTDx = MTDx + MLSpacing + self.WdgParDict[MAx][0] + 2*self.WdgParDict[MAx][3]
            return 1
    
    def CreateSingleTurn(self,PathX,PathZ,ProfTyp,ProfAX,ProfZ,ZPos,LayNum,TurnNum,SAng,funcLib):
        NumSegs = int(360/SAng)
    ...
    def DrawBobbin(self,Hb,Db1,Db2,Tb,SAng,funcLib):  
        NumSegs = int(360/SAng)
