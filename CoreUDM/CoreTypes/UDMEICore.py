...
#                           Default Parameters
##############################################################
udmInfo = UDMInfo( name = "EI_Core",
                    company = "ANSYS, Inc.",
                    purpose = "Create EI Core and Windings",
...
#---------------------------------------------------------------------------------       
    def Refresh(self, funcLib, udmInParams, updatedParams, refreshModifiedPartsOnly, nonEditedPartRefIds):
        ...
        if DoAirgap == 1:
            ...
            if AirGapOn ==1:
                AirGapC = self._GetParamValueByName('Airgap Value')
                AirGapS = 0
                TAirGap = 0
            elif AirGapOn ==2:
                AirGapS = self._GetParamValueByName('Airgap Value')
                AirGapC = 0
                TAirGap = 0
            else:
                AirGapS = 0
        ...
        Leg2Pos = UDPPosition(-MCLegWidth/2,-(MECoreWidth)/2,-MECoreHeight-TAirGap)
        ...
        CLegSize = [MCLegWidth, MECoreWidth, MECoreHeight-AirGapC]
        ...
        posEdge1 = UDPPosition(-DimD1/2, -DimD6/2, -(DimD4/2)-TAirGap)
        edgeID1 = funcLib.GetEdgeIDFromPosition(posEdge1)
        posEdge2 = UDPPosition(-DimD1/2, DimD6/2, -(DimD4/2)-TAirGap)
        edgeID2 = funcLib.GetEdgeIDFromPosition(posEdge2)
        posEdge3 = UDPPosition(DimD1/2, -DimD6/2, -(DimD4/2)-TAirGap)
        edgeID3 = funcLib.GetEdgeIDFromPosition(posEdge3)
        posEdge4 = UDPPosition(DimD1/2, DimD6/2, -(DimD4/2)-TAirGap)
        edgeID4 = funcLib.GetEdgeIDFromPosition(posEdge4)
        CblendEles = UDPBLNDElements(XBaseCore)
        listOfEdges = [edgeID1,edgeID2,edgeID3,edgeID4]
        CblendEles.ListOfEdges = listOfEdges
        CblendOpts = UDPBLNDFilletOptions(True, BLNDFilletRadiusLaw.BLNDConstantRadius, DimD7, 0.0, True, BLNDFilletType.BLNDRound, 0.0, 0.0)
        fCore = funcLib.Fillet(CblendEles, CblendOpts)
        MovVectE = UDPVector(0,0,(DimD5/2))
        funcLib.Translate(XBaseCore,MovVectE)
        
        funcLib.SetPartName("E_Core",XBaseCore)
        funcLib.SetPartColor(XBaseCore,0xa52a2a)
        MyUDMUtils.SetAttributes(XBaseCore, "E_Core", funcLib)
        #MyUDMUtils.SetPartAttributes(XBaseCore,"E_Core", "ferrite", 0xa52a2a, funcLib, "E_Core")
        
        ICorPos = UDPPosition(-DimD1/2, -DimD6/2, TAirGap)
        ICorSize = [ DimD1,DimD6,DimD8]
        ICore = funcLib.CreateBox(ICorPos, ICorSize)
        
        posEdge5 = UDPPosition(-DimD1/2, -DimD6/2, (DimD8/2)+TAirGap)
        edgeID5 = funcLib.GetEdgeIDFromPosition(posEdge5)
        posEdge6 = UDPPosition(-DimD1/2, DimD6/2, (DimD8/2)+TAirGap)
        edgeID6 = funcLib.GetEdgeIDFromPosition(posEdge6)
        posEdge7 = UDPPosition(DimD1/2, -DimD6/2, (DimD8/2)+TAirGap)
        edgeID7 = funcLib.GetEdgeIDFromPosition(posEdge7)
        posEdge8 = UDPPosition(DimD1/2, DimD6/2, (DimD8/2)+TAirGap)
        edgeID8 = funcLib.GetEdgeIDFromPosition(posEdge8)
        CblendEles2 = UDPBLNDElements(ICore)
        listOfEdges2 = [edgeID5,edgeID6,edgeID7,edgeID8]
        CblendEles2.ListOfEdges = listOfEdges2
        CblendOpts2 = UDPBLNDFilletOptions(True, BLNDFilletRadiusLaw.BLNDConstantRadius, DimD7, 0.0, True, BLNDFilletType.BLNDRound, 0.0, 0.0)
        fCore2 = funcLib.Fillet(CblendEles2, CblendOpts2)
        funcLib.Translate(ICore,MovVectE)
        
        funcLib.SetPartName("I_Core",ICore)
        funcLib.SetPartColor(ICore,0xa52a2a)
        MyUDMUtils.SetAttributes(ICore, "I_Core", funcLib)
        #MyUDMUtils.SetPartAttributes(ICore,"I_Core", "ferrite", 0xa52a2a, funcLib, "I_Core")
        
        CreateWdg = self._GetParamValueByName('WdgStatus')
        if CreateWdg == 1:
            if self.DrawWdg(DimD2,DimD3,DimD5+2*TAirGap,DimD6,SAng,funcLib) == 0:
...
#-----------------------------------------------------------------------
    ...
    def DrawWdg(self,DimD2,DimD3,DimD5,DimD6,SAng,funcLib):
        ...
        MSlotHeight = DimD5
        if BobStat > 0:
            self.DrawBobbin(MSlotHeight-2*MTopMargin,(DimD2/2.0)-MSideMargin,(DimD3/2.0)+MSideMargin+MBobbinThk,(DimD6/2.0)+MSideMargin+MBobbinThk,MBobbinThk,MSideMargin,SAng,funcLib)
        ...
        if MWdgType == 1:
            ...
            for MAx in self.WdgParDict:
                for MBx in range(0,int(self.WdgParDict[MAx][2])):
                    ...
                    if MCondType == 1:
                        ...
                        self.CreateSingleTurn2(MRecSzX,MRecSzY,MRecSzZ,MCondType,self.WdgParDict[MAx][0],self.WdgParDict[MAx][1],(DimD5/2)-MTDx-(self.WdgParDict[MAx][3]),MAx,MBx,(MRecSzX-DimD3)/2,SAng,funcLib)
                    else:
                        MRecSzZ = -self.WdgParDict[MAx][0]/2.0
                        self.CreateSingleTurn(MRecSzX,MRecSzY,MRecSzZ,MCondType,self.WdgParDict[MAx][0],self.WdgParDict[MAx][1],(DimD5/2)-MTDx-(self.WdgParDict[MAx][3]),MAx,MBx,(MRecSzX-DimD3)/2,SAng,funcLib)
                if MCondType == 1:
            ...
            for MAx in self.WdgParDict.keys():
                for MBx in range(0,int(self.WdgParDict[MAx][2])):
                    ...
                    if MCondType == 1:
                        MRecSzZ = -self.WdgParDict[MAx][1]/2.0
                        self.CreateSingleTurn2(MRecSzX,MRecSzY,MRecSzZ,MCondType,self.WdgParDict[MAx][0],self.WdgParDict[MAx][1],(DimD5/2)-MTopMargin-MBobbinThk-(self.WdgParDict[MAx][3])-MBx*(2*self.WdgParDict[MAx][3]+self.WdgParDict[MAx][1]),MAx,MBx,(MRecSzX-DimD3)/2,SAng,funcLib)
                    else:
                        MRecSzZ = -self.WdgParDict[MAx][0]/2.0
                        self.CreateSingleTurn(MRecSzX,MRecSzY,MRecSzZ,MCondType,self.WdgParDict[MAx][0],self.WdgParDict[MAx][1],(DimD5/2)-MTopMargin-MBobbinThk-(self.WdgParDict[MAx][3])-MBx*(2*self.WdgParDict[MAx][3]+self.WdgParDict[MAx][0]),MAx,MBx,(MRecSzX-DimD3)/2,SAng,funcLib)
                MTDx = MTDx + MLSpacing + self.WdgParDict[MAx][0] + 2*self.WdgParDict[MAx][3]
            return 1
    
    def CreateSingleTurn(self,PathX,PathY,PathZ,ProfTyp,ProfAX,ProfZ,ZPos,LayNum,TurnNum,FRad,SAng,funcLib):
        ...
        for cnt2 in range(0,NumSegs-1):
            ...
            vertices[2+cnt2].Y = -(PathY/2.0) +FRad - FRad*math.cos(SegAng*(cnt2+1))
            vertices[2+cnt2].Z = PathZ
        
        for cnt3 in range(0,NumSegs-1):
            vertices[3+NumSegs+cnt3].X = -(PathX/2.0) +FRad - FRad*math.cos(SegAng*(cnt3+1))
            vertices[3+NumSegs+cnt3].Y = (PathY/2.0) -FRad + FRad*math.sin(SegAng*(cnt3+1))
        ...
        for cnt4 in range(0,NumSegs-1):
            ...
            vertices[4+2*NumSegs+cnt4].Y = (PathY/2.0) -FRad + FRad*math.cos(SegAng*(cnt4+1))
    ...
    def CreateSingleTurn2(self,PathX,PathY,PathZ,ProfTyp,ProfAX,ProfZ,ZPos,LayNum,TurnNum,FRad,SAng,funcLib):
