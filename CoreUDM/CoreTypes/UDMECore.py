...
#                           Default Parameters
##############################################################
udmInfo = UDMInfo( name = "E_Core",
                    company = "ANSYS, Inc.",
                    purpose = "Create E Core and Windings",
...
#---------------------------------------------------------------------------------       
    def Refresh(self, funcLib, udmInParams, updatedParams, refreshModifiedPartsOnly, nonEditedPartRefIds):
        ...
        posEdge3 = UDPPosition(-DimD2/2, 0, -DimD5-TAirGap)
        edgeID3 = funcLib.GetEdgeIDFromPosition(posEdge3)
        posEdge4 = UDPPosition(DimD2/2, 0, -DimD5-TAirGap)
        edgeID4 = funcLib.GetEdgeIDFromPosition(posEdge4)
        CblendEles2 = UDPBLNDElements(XBaseCore)
        listOfEdges2 = [edgeID3,edgeID4]
        CblendEles2.ListOfEdges = listOfEdges2
        CblendOpts2 = UDPBLNDFilletOptions(True, BLNDFilletRadiusLaw.BLNDConstantRadius, DimD8, 0.0, True, BLNDFilletType.BLNDRound, 0.0, 0.0)
        fCore2 = funcLib.Fillet(CblendEles2, CblendOpts2)
        
        funcLib.SetPartName("E_Core_Bottom",XBaseCore)
        funcLib.SetPartColor(XBaseCore,0xa52a2a)
        MyUDMUtils.SetAttributes(XBaseCore, "E_Core_Bottom", funcLib)
        
        #MyUDMUtils.SetPartAttributes(XBaseCore,"E_Core_Bottom", "ferrite", 0xa52a2a, funcLib, "E_Core_Bottom")
        MirrorVect = UDPVector(0,0,1)
        MirrorPt = UDPPosition(0,0,0)
        TopCore = funcLib.DuplicateAndMirror(XBaseCore,MirrorPt,MirrorVect)
        
        funcLib.SetPartName("E_Core_Top",TopCore)
        funcLib.SetPartColor(TopCore,0xa52a2a)
        MyUDMUtils.SetAttributes(TopCore, "E_Core_Top", funcLib)
        #MyUDMUtils.SetPartAttributes(TopCore,"E_Core_Top", "ferrite", 0xa52a2a, funcLib, "E_Core_Top")
        CreateWdg = self._GetParamValueByName('WdgStatus')
        if CreateWdg == 1:
            if self.DrawWdg(DimD2,DimD3,DimD5+TAirGap,DimD6,SAng,funcLib) == 0:
        ...
        if MWdgType == 1:
            ...
            for MAx in self.WdgParDict:
                for MBx in range(0,int(self.WdgParDict[MAx][2])):
                    ...
                    if MCondType == 1:
                        ...
                        self.CreateSingleTurn2(MRecSzX,MRecSzY,MRecSzZ,MCondType,self.WdgParDict[MAx][0],self.WdgParDict[MAx][1],DimD5-MTDx-(self.WdgParDict[MAx][3]),MAx,MBx,(MRecSzX-DimD3)/2,SAng,funcLib)
                    else:
                        MRecSzZ = -self.WdgParDict[MAx][0]/2.0
                        self.CreateSingleTurn(MRecSzX,MRecSzY,MRecSzZ,MCondType,self.WdgParDict[MAx][0],self.WdgParDict[MAx][1],DimD5-MTDx-(self.WdgParDict[MAx][3]),MAx,MBx,(MRecSzX-DimD3)/2,SAng,funcLib)
                if MCondType == 1:
            ...
            for MAx in self.WdgParDict.keys():
                for MBx in range(0,int(self.WdgParDict[MAx][2])):
                    ...
                    if MCondType == 1:
                        MRecSzZ = -self.WdgParDict[MAx][1]/2.0
                        self.CreateSingleTurn2(MRecSzX,MRecSzY,MRecSzZ,MCondType,self.WdgParDict[MAx][0],self.WdgParDict[MAx][1],DimD5-MTopMargin-MBobbinThk-(self.WdgParDict[MAx][3])-MBx*(2*self.WdgParDict[MAx][3]+self.WdgParDict[MAx][1]),MAx,MBx,(MRecSzX-DimD3)/2,SAng,funcLib)
                    else:
                        MRecSzZ = -self.WdgParDict[MAx][0]/2.0
                        self.CreateSingleTurn(MRecSzX,MRecSzY,MRecSzZ,MCondType,self.WdgParDict[MAx][0],self.WdgParDict[MAx][1],DimD5-MTopMargin-MBobbinThk-(self.WdgParDict[MAx][3])-MBx*(2*self.WdgParDict[MAx][3]+self.WdgParDict[MAx][0]),MAx,MBx,(MRecSzX-DimD3)/2,SAng,funcLib)
                MTDx = MTDx + MLSpacing + self.WdgParDict[MAx][0] + 2*self.WdgParDict[MAx][3]
            return 1
