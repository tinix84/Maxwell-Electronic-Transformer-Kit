...
#                           Default Parameters
##############################################################
udmInfo = UDMInfo(name="EC_Core",
                  company="ANSYS, Inc.",
                  purpose="Create EC Core and Windings",
...
class UDMExtension(IUDMExtension):
    ...
    def Refresh(self, funcLib, udmInParams, updatedParams, refreshModifiedPartsOnly, nonEditedPartRefIds):
        ...
        if self.CheckCoreDim([DimD1, DimD2, DimD3, DimD4, DimD5, DimD6]) == 0:
        ...
        Cyl1Pos = UDPPosition(0, 0, -MSlotDepth - TAirGap)
        Cyl1Sta = UDPPosition(-DimD2 / 2, 0, -MSlotDepth - TAirGap)
        ...
        Cyl2Sta = UDPPosition(-DimD3 / 2, 0, -MSlotDepth - TAirGap)
        ...
        funcLib.SetPartName("EC_Core_Bottom", XBaseCore)
        funcLib.SetPartColor(XBaseCore, 0xa52a2a)
        MyUDMUtils.SetAttributes(XBaseCore, "EC_Core_Bottom", funcLib)

        # MyUDMUtils.SetPartAttributes(XBaseCore,"EC_Core_Bottom", "ferrite", 0xa52a2a, funcLib, "EC_Core_Bottom")
        MirrorVect = UDPVector(0, 0, 1)
        MirrorPt = UDPPosition(0, 0, 0)
        TopCore = funcLib.DuplicateAndMirror(XBaseCore, MirrorPt, MirrorVect)

        funcLib.SetPartName("EC_Core_Top", TopCore)
        funcLib.SetPartColor(TopCore, 0xa52a2a)
        MyUDMUtils.SetAttributes(TopCore, "EC_Core_Top", funcLib)
        # MyUDMUtils.SetPartAttributes(TopCore,"EC_Core_Top", "ferrite", 0xa52a2a, funcLib, "EC_Core_Top")
        ...
        if WdgParamDict['WdgStatus'] > 0:
            ...
            for EachLayer in InList:
                ...
                for EachLayProp in range(1, len(LayerLst)):
                    if WdgParamDict['CondType'] == 1:
                        if EachLayProp == 3:
                            LayerPropDict[LayerLst[0] + " " + LayerPropList[EachLayProp - 1]] = int(
                                LayerLst[EachLayProp])
                        else:
                            LayerPropDict[LayerLst[0] + " " + LayerPropList[EachLayProp - 1]] = float(
                                LayerLst[EachLayProp]) * SF
                    else:
    ...
    def DrawWdg(self, DimD2, DimD3, DimD5, DimD6, SAng, funcLib):
        ...
        MSlotHeight = DimD5 * 2
        if BobStat > 0:
            self.DrawBobbin(MSlotHeight - 2 * MTopMargin, (DimD2 / 2.0) - MSideMargin,
                            (DimD3 / 2.0) + MSideMargin + MBobbinThk, MBobbinThk, SAng, funcLib)
        for ELay in range(0, MNumLayers):
            ...
            if MCondType == 1:
                ...
                self.WdgParDict[ELay + 1].append(
                    self._GetParamValueByName('Layer_%s Insulation Thickness' % (ELay + 1)))
            else:
        ...
        if MWdgType == 1:
            ...
            for MAx in self.WdgParDict:
                for MBx in range(0, int(self.WdgParDict[MAx][2])):
                    ...
                    self.CreateSingleTurn(MRecSzX, MRecSzZ, MCondType, self.WdgParDict[MAx][0], self.WdgParDict[MAx][1],
                                          DimD5 - MTDx - (self.WdgParDict[MAx][3]), MAx, MBx, SAng, funcLib)
                if MCondType == 1:
            ...
            for MAx in self.WdgParDict.keys():
                for MBx in range(0, int(self.WdgParDict[MAx][2])):
                    ...
                    if MCondType == 1:
                        MRecSzZ = -self.WdgParDict[MAx][1] / 2.0
                        self.CreateSingleTurn(MRecSzX, MRecSzZ, MCondType, self.WdgParDict[MAx][0],
                                              self.WdgParDict[MAx][1],
                                              DimD5 - MTopMargin - MBobbinThk - (self.WdgParDict[MAx][3]) - MBx * (
                                                          2 * self.WdgParDict[MAx][3] + self.WdgParDict[MAx][1]), MAx,
                                              MBx, SAng, funcLib)
                    else:
                        MRecSzZ = -self.WdgParDict[MAx][0] / 2.0
                        self.CreateSingleTurn(MRecSzX, MRecSzZ, MCondType, self.WdgParDict[MAx][0],
                                              self.WdgParDict[MAx][1],
                                              DimD5 - MTopMargin - MBobbinThk - (self.WdgParDict[MAx][3]) - MBx * (
                                                          2 * self.WdgParDict[MAx][3] + self.WdgParDict[MAx][0]), MAx,
                                              MBx, SAng, funcLib)
                MTDx = MTDx + MLSpacing + self.WdgParDict[MAx][0] + 2 * self.WdgParDict[MAx][3]
            return 1
    ...
    def ParCheckTopDown(self, CSideMargin, CTopMargin, CLSpacing, CNumLayers, CSlotWidth, CSlotHeight, CCondType,
                        funcLib):
        WdgStackHieght = 0.0
        WdgStackWidth = []
        for EAStack in self.WdgParDict:
            if CCondType == 1:
                ...
                WdgStackWidth.append(CSideMargin + 2 * self.WdgParDict[EAStack][3] + (
                            self.WdgParDict[EAStack][0] * self.WdgParDict[EAStack][2]) + (
                                                 2 * self.WdgParDict[EAStack][3] * (self.WdgParDict[EAStack][2])))
            else:
        ...
        if (CTopMargin + ((CNumLayers - 1) * CLSpacing) + WdgStackHieght) > CSlotHeight:
            funcLib.AddMessage(MessageSeverity.ErrorMessage,
                               "Core slot can not accommodate all winding layers.Please check your inputs.")
            return 0
        if max(WdgStackWidth) > CSlotWidth:
            funcLib.AddMessage(MessageSeverity.ErrorMessage,
                               "Core slot can not accommodate all winding turns. Please check your inputs.")
            return 0
        return 1

    def ParCheckConcen(self, CSideMargin, CTopMargin, CLSpacing, CNumLayers, CSlotWidth, CSlotHeight, CCondType,
                       funcLib):
        WdgStackHieght = []
        WdgStackWidth = 0.0
        for EAStack in self.WdgParDict:
            if CCondType == 1:
                WdgStackHieght.append(CTopMargin + (self.WdgParDict[EAStack][1] * self.WdgParDict[EAStack][2]) + (
                            2 * self.WdgParDict[EAStack][3] * (self.WdgParDict[EAStack][2])))
            else:
                WdgStackHieght.append(CTopMargin + (self.WdgParDict[EAStack][0] * self.WdgParDict[EAStack][2]) + (
                            2 * self.WdgParDict[EAStack][3] * (self.WdgParDict[EAStack][2])))
