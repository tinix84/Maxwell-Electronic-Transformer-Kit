...
#                           Default Parameters
##############################################################
udmInfo = UDMInfo( name = "PQ_Core",
                    company = "ANSYS, Inc.",
                    purpose = "Create PQ Core and Windings",
...
#---------------------------------------------------------------------------------       
    def Refresh(self, funcLib, udmInParams, updatedParams, refreshModifiedPartsOnly, nonEditedPartRefIds):
        ...
        BaseCorePos = UDPPosition(-DimD1/2,-DimD8/2,-(DimD4/2)-TAirGap)
        BaseCoreSz= [(DimD1-DimD6)/2, DimD8, (DimD4/2)-AirGapS]
        XBaseCore = funcLib.CreateBox(BaseCorePos, BaseCoreSz)
        ...
        IntL = math.sqrt(((DimD3/2)**2)-((DimD7/2)**2))
        
        vertices.append(UDPPosition(-DimD6/2,-0.4*DimD8,-(DimD4/2)-TAirGap))
        vertices.append(UDPPosition(-DimD6/2,0.4*DimD8,-(DimD4/2)-TAirGap))
        vertices.append(UDPPosition(-DimD7/2,+IntL,-(DimD4/2)-TAirGap))
        vertices.append(UDPPosition(-DimD7/2,-IntL,-(DimD4/2)-TAirGap))
        #vertices.append(UDPPosition(-DimD2/2,-0.9*DimD8/2,-DimD4/2))
        
        polylineDefinition = UDPPolylineDefinition(vertices,arrayOfSegmentDefinition,1,1)
        Prof = funcLib.CreatePolyline(polylineDefinition)
        
        SweepVect = UDPVector(0,0,(DimD4/2)-AirGapS)
        sweepOptions = UDPSweepOptions(SweepDraftType.RoundDraft, 0.0, 0.0)
        
        XBase2 = funcLib.SweepAlongVector(Prof,SweepVect,sweepOptions)
        
        funcLib.Unite([XBaseCore,Prof])
        ...
        MirrorVect = UDPVector(1,0,0)
        MirrorPt = UDPPosition(0,0,0)
        XBaseCore2 = funcLib.DuplicateAndMirror(XBaseCore,MirrorPt,MirrorVect)
        
        Cyl2Pos = UDPPosition(0,0,-(DimD4/2)-TAirGap)
        Cyl2Sta = UDPPosition(-(DimD3/2),0,-(DimD4/2)-TAirGap)
        XCyl2 = funcLib.CreatePolyhedron(CoordinateSystemAxis.ZAxis,Cyl2Pos, Cyl2Sta, NumSegs,(DimD4/2)-AirGapC)
            
        funcLib.Unite([XBaseCore,XBaseCore2,XCyl2])        
        
        funcLib.SetPartName("PQ_Core_Bottom",XBaseCore)
        funcLib.SetPartColor(XBaseCore,0xa52a2a)
        MyUDMUtils.SetAttributes(XBaseCore, "PQ_Core_Bottom", funcLib)
        #MyUDMUtils.SetPartAttributes(XBaseCore,"PQ_Core_Bottom", "ferrite", 0xa52a2a, funcLib, "PQ_Core_Bottom")
        MirrorVect2 = UDPVector(0,0,1)
        MirrorPt2 = UDPPosition(0,0,0)
        TopCore = funcLib.DuplicateAndMirror(XBaseCore,MirrorPt2,MirrorVect2)
        
        funcLib.SetPartName("PQ_Core_Top",TopCore)
        funcLib.SetPartColor(TopCore,0xa52a2a)
        MyUDMUtils.SetAttributes(TopCore, "PQ_Core_Top", funcLib)
        #MyUDMUtils.SetPartAttributes(TopCore,"PQ_Core_Top", "ferrite", 0xa52a2a, funcLib, "PQ_Core_Top")
